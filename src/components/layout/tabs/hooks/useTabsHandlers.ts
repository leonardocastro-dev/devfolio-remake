...
export function useTabsHandlers({ tabs, setActiveTab, reorderTabs }: TabsHandlersProps) {
  ...
  const handleDragStart = useCallback((e: React.DragEvent, index: number) => {
    ...
    e.dataTransfer.setDragImage(dragImageEl, DRAG_IMAGE_POSITION.OFFSET_X, DRAG_IMAGE_POSITION.OFFSET_Y)
  }, [tabs, setActiveTab])
  ...
  const handleDragEnter = useCallback((e: React.DragEvent, enterIndex: number) => {
    e.preventDefault()
